function [u,t]=bernus_RL(num_steps)  
...
for i=2:length(t)
    ...
    vn=exp(Lo*tau)*vo+1/Lo*(exp(Lo*tau)-1)*No;  %RL
    ...
    fn=exp(Lo*tau)*fo+1/Lo*(exp(Lo*tau)-1)*No;  %RL

    
    Lo=-alpha_to( uo )-beta_to( uo );
    No=alpha_to( uo );
    
    ton=exp(Lo*tau)*too+1/Lo*(exp(Lo*tau)-1)*No;  %RL

    Lo=-1/tau_X( uo );
    No=X_inf( uo )/tau_X( uo) ;
    Xn=exp(Lo*tau)*Xo+1/Lo*(exp(Lo*tau)-1)*No;  %RL
