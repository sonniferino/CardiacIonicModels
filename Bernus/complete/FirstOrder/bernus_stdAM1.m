function [u,t]=bernus_stdAM1(num_steps)  
...
for i=2:length(t)
    ...
    vn= (vo+tau*N)/(1-tau*L); %std AM1

   
    L=-alpha_m(un)-beta_m( un );
    N=alpha_m( un );
    
    mn= (mo+tau*N)/(1-tau*L); %std AM1


    L=-alpha_f( un )-beta_f(un );
    N=alpha_f(un);
     
    fn= (fo+tau*N)/(1-tau*L); %std AM1

    
    L=-alpha_to( un )-beta_to( un );
    N=alpha_to( un );
    
    ton= (too+tau*N)/(1-tau*L); %std AM1
    
    L=-1/tau_X( un );
 
    N=X_inf( un )/tau_X( un) ;    
    
	Xn= (Xo+tau*N)/(1-tau*L); %std AM1
