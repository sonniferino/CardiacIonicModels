function [u,t]=bernus_stdAB2(num_steps)  
...
for i=3:length(t)
   ...
   % us=uo-dt*(I_tot_o);
    un=uo-tau*((3/2)*I_tot_o +(-1/2)*I_tot_older); %AB2 lo terrei fisso ma qui non serve
    ...
    vn= vo+tau*((3/2)*(Lo*vo+No)-(1/2)*(Lolder*volder+Nolder)); %std AB2
    ...
    mn= mo+tau*((3/2)*(Lo*mo+No)-(1/2)*(Lolder*molder+Nolder)); %std AB2
    ...
    fn= fo+tau*((3/2)*(Lo*fo+No)-(1/2)*(Lolder*folder+Nolder)); %std AB2
    ...
    ton= too+tau*((3/2)*(Lo*too+No)-(1/2)*(Lolder*toolder+Nolder)); %std AB2
    ...
    Xn= Xo+tau*((3/2)*(Lo*Xo+No)-(1/2)*(Lolder*Xolder+Nolder)); %std AB2
