function [u,t,I_Na,I_Ca,I_to, I_K,I_K1,I_Nab,I_Cab,I_NaK,I_NaCa]=bernus_expexp(num_steps)
...
for i=2:length(t)   %la forza e' che non usa older
    ...
    us=uo-tau*(I_tot_o);   %AB1
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
    Lo=-1/tau_v(uo);
    N=v_inf(uo)/tau_v(uo);
    %vs=exp(Lo*tau)*vo+1/Lo*(exp(Lo*tau)-1)*N;  %RL1 com'era prima 
     
    L=-1/tau_v(us);                  % aggiunto da me
    Lp= 0.5*tau*( Lo + L );          % aggiunto da me
    vs= exp(Lp)*vo+tau*N;   %IFAM1   % aggiunto da me
   
    Lo=-alpha_m(uo)-beta_m(uo);
    N=alpha_m(uo);
   % ms=exp(Lo*tau)*mo+1/Lo*(exp(Lo*tau)-1)*N;  %% quella problematica ? la
   % m %RL1 com'era prima
   
    L=-alpha_m(us)-beta_m(us);       % aggiunto da me
    Lp= 0.5*tau*( Lo + L );          % aggiunto da me
    ms= exp(Lp)*mo+tau*N;   %IFAM1   % aggiunto da me
   
   
    Lo=-alpha_f(uo)-beta_f(uo);
    N=alpha_f(uo);
    %fs=exp(Lo*tau)*fo+1/Lo*(exp(Lo*tau)-1)*N; %RL1 com'era prima
        
    L=-alpha_f(us)-beta_f(us);       % aggiunto da me
    Lp= 0.5*tau*( Lo + L );          % aggiunto da me
    fs= exp(Lp)*fo+tau*N;   %IFAM1   % aggiunto da me
    
    
    Lo=-alpha_to(uo)-beta_to(uo);
    N=alpha_to(uo);
    %tos=exp(Lo*tau)*too+1/Lo*(exp(Lo*tau)-1)*N; %RL1 com'era prima 
    
    L=-alpha_to(us)-beta_to(us);       % aggiunto da me
    Lp= 0.5*tau*( Lo + L );          % aggiunto da me
    tos= exp(Lp)*too+tau*N;   %IFAM1   % aggiunto da me
    
    
    Lo=-1/tau_X(uo);
    N=X_inf(uo)/tau_X(uo);
   % Xs=exp(Lo*tau)*Xo+1/Lo*(exp(Lo*tau)-1)*N;  %RL1 com'era prima
   
   
    L=-1/tau_X(us);       % aggiunto da me
    Lp= 0.5*tau*( Lo + L );          % aggiunto da me
    Xs= exp(Lp)*Xo+tau*N;   %IFAM1   % aggiunto da me
   
    
    %%%%%%%%%%%%%%%%%   AGGIUNTO  %%%%%%%%%%%%%%%%%%%%%
    
    Lo=-1/tau_v( 0.5*(uo+us) );
    N=v_inf(0.5*(uo+us))/tau_v(0.5*(uo+us));
    vn=exp(Lo*tau)*vo+1/Lo*(exp(Lo*tau)-1)*N;
    
    Lo=-alpha_m( 0.5*(uo+us) )-beta_m( 0.5*(uo+us) );
    N=alpha_m( 0.5*(uo+us) );
    mn=exp(Lo*tau)*mo+1/Lo*(exp(Lo*tau)-1)*N;  %% quella problematica ? la m
    
    Lo=-alpha_f( 0.5*(uo+us) )-beta_f( 0.5*(uo+us) );
    N=alpha_f( 0.5*(uo+us) );
    fn=exp(Lo*tau)*fo+1/Lo*(exp(Lo*tau)-1)*N;
    
    Lo=-alpha_to( 0.5*(uo+us) )-beta_to( 0.5*(uo+us) );
    N=alpha_to( 0.5*(uo+us) );
    ton=exp(Lo*tau)*too+1/Lo*(exp(Lo*tau)-1)*N;
    
    Lo=-1/tau_X( 0.5*(uo+us) );
    N=X_inf( 0.5*(uo+us) )/tau_X( 0.5*(uo+us) );
    Xn=exp(Lo*tau)*Xo+1/Lo*(exp(Lo*tau)-1)*N;
    
   %%%%%%%%%% correction phase
   
    I_Na_s=I_Na_fun( 0.5*(uo+us) , 0.5*(mo+ms) , 0.5*(vo+vs) );
    I_Ca_s=I_Ca_fun( 0.5*(uo+us) , 0.5*(fo+fs) );
    I_to_s=I_to_fun( 0.5*(uo+us) , 0.5*(too+tos) );
    I_K_s=I_K_fun( 0.5*(uo+us) , 0.5*(Xo+Xs) );
    I_K1_s=I_K1_fun( 0.5*(uo+us) );
    I_Nab_s=I_Nab_fun(  0.5*(uo+us)  );
    I_Cab_s=I_Cab_fun( 0.5*(uo+us) );
    I_NaK_s=I_NaK_fun( 0.5*(uo+us) );
    I_NaCa_s=I_NaCa_fun( 0.5*(uo+us) );
    
    un=uo-tau*(I_Na_s+I_Ca_s+I_to_s+I_K_s+I_K1_s+I_Nab_s+I_Cab_s+I_NaK_s+I_NaCa_s);
