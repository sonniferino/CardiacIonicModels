function [u,t]=bernus_ExpSecondOrder(num_steps)  
...
%     %%%%% try to correct it after have it predicted! %%%
%     %%%%%%%%%%%%%%%%% exponential AB1 %%%%%%%%%%%%%%%%%%%%%
%     
%     L=-1/tau_v( un );
%     Lo=-1/tau_v( uo );
...
for i=3:length(t)
   ...
   % us=uo-dt*(I_tot_o);
    un=uo-tau*((3/2)*I_tot_o +(-1/2)*I_tot_older); %AB2 lo terrei fisso
    ...
    vn= exp(Lp)*vo+(3/2)*tau*exp(Lp)*No+(-1/2)*tau*exp(Lg)*Nolder;   %IFAB2
    %vn= exp(Lp)*vo+(tau/2)*(N+exp(Lp)*No);   %IFAM2 con RL converge sempre con ordine 2 
    ...
    mn= exp(Lp)*mo+(3/2)*tau*exp(Lp)*No+(-1/2)*tau*exp(Lg)*Nolder;  %IFAB2
    ...
    fn= exp(Lp)*fo+(3/2)*tau*exp(Lp)*No+(-1/2)*tau*exp(Lg)*Nolder; %IFAB2
    ...
    ton= exp(Lp)*too+(3/2)*tau*exp(Lp)*No+(-1/2)*tau*exp(Lg)*Nolder;  %IFAB2 
    ...
    %Xn= exp(Lp)*Xo+(3/2)*tau*exp(Lp)*No+(-1/2)*tau*exp(Lg)*Nolder;   %IFAB2
...
%     Lg= tau*0.5*( Lolder + Lo ) + Lp;
%     
%     vn= exp(Lp)*vo+(3/2)*tau*exp(Lp)*No+(-1/2)*tau*exp(Lg)*Nolder; 
%     
%     
%     
%     L=-alpha_m(un)-beta_m( un );
...
%     Lg= tau*0.5*( Lolder + Lo ) + Lp;
%     
%     mn= exp(Lp)*mo+(3/2)*tau*exp(Lp)*No+(-1/2)*tau*exp(Lg)*Nolder; 
% 
%    
%     L=-alpha_f( un )-beta_f(un );
%     Lo=-alpha_f( uo )-beta_f(uo );
...
%     Lg= tau*0.5*( Lolder + Lo ) + Lp;
%     
%     fn= exp(Lp)*fo+(3/2)*tau*exp(Lp)*No+(-1/2)*tau*exp(Lg)*Nolder; 
%     
%     
%     L=-alpha_to( un )-beta_to( un );
%     Lo=-alpha_to( uo )-beta_to( uo );
...
%     Lg= tau*0.5*( Lolder + Lo ) + Lp;
%     
%     ton= exp(Lp)*too+(3/2)*tau*exp(Lp)*No+(-1/2)*tau*exp(Lg)*Nolder;   
%     
%     
%     L=-1/tau_X( un );
%     Lo=-1/tau_X( uo );
...
%     Lg= tau*0.5*( Lolder + Lo ) + Lp;
%     
%     Xn= exp(Lp)*Xo+(3/2)*tau*exp(Lp)*No+(-1/2)*tau*exp(Lg)*Nolder;   
%  
%     
%     I_Na_n=I_Na_fun( un , mn , vn );
%     I_Ca_n=I_Ca_fun( un,fn);
