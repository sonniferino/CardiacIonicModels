function [u,t]=bernus_IFBDF2(num_steps)  
...
for i=3:length(t)
   ...
   vn=4/3*exp(Lp)*vo-1/3*exp(Lg)*volder +tau*2/3*N ; %IFBDF2
    ...
    mn=4/3*exp(Lp)*mo-1/3*exp(Lg)*molder +tau*2/3*N ; %IFBDF2
    ...
    fn=4/3*exp(Lp)*fo-1/3*exp(Lg)*folder +tau*2/3*N ; %IFBDF2
    ...
    ton=4/3*exp(Lp)*too-1/3*exp(Lg)*toolder +tau*2/3*N ; %IFBDF2
   ...
   Xn=4/3*exp(Lp)*Xo-1/3*exp(Lg)*Xolder +tau*2/3*N ; %IFBDF2
