function [u,t]=bernus_TEtau3(num_steps,uref,tref, I_Naref,I_Caref,I_toref, I_Kref,I_K1ref,I_Nabref,I_Cabref,I_NaKref,I_NaCaref)
%)  
...
for i=2:2
    ...
    %%%%%%%% troviamo una guess per un %%%%%%%%
    
    us=uo-tau*(I_tot_o); %passo esplicito
    ...
    Lp= 0.5*tau*(Lo + L );  
    vs= exp(Lp)*vo+tau*exp(Lp)*No; %e' IFAB1
   
    %vs=exp(L*tau)*vo+1/L*(exp(L*tau)-1)*N;  %e' RL
    %vs =vo + tau *v_fun(vo,uo); %e' AB1
    ...
    Lp= 0.5*tau*(Lo + L );  
    ms= exp(Lp)*mo+tau*exp(Lp)*No; %e' IFAB1
   % ms=exp(L*tau)*mo+1/L*(exp(L*tau)-1)*N;  %% quella problematica ? la m
   % ms =mo + tau *m_fun(mo,uo); %e' AB1
    ...
    No=alpha_f(uo);
    Lp= 0.5*tau*(Lo + L );  
    fs= exp(Lp)*fo+tau*exp(Lp)*No; %e' IFAB1
    %fs=exp(L*tau)*fo+1/L*(exp(L*tau)-1)*N;
   % fs =fo + tau *f_fun(fo,uo); %e' AB1
    ...
    Lp= 0.5*tau*(Lo + L );  
    tos= exp(Lp)*too+tau*exp(Lp)*No; %e' IFAB1
    % tos=exp(L*tau)*too+1/L*(exp(L*tau)-1)*N;
    %tos=too+ tau *to_fun(too,uo); %e' AB1
    ...
    Lp= 0.5*tau*(Lo + L );  
    Xs= exp(Lp)*Xo+tau*exp(Lp)*No; %e' IFAB1
   % Xs=exp(L*tau)*Xo+1/L*(exp(L*tau)-1)*N;  %RL sulle gating
   % Xs= Xo + tau *X_fun(Xo,uo); %e' AB1
    ...
    N=v_inf(us)/tau_v(us);
    No=v_inf(uo)/tau_v(uo);
    %vn=exp(L*tau)*vo+1/L*(exp(L*tau)-1)*N;
    %vn =vo + (tau/2) *(v_fun(vo,uo)+v_fun(vs,us));  % AM2
    vn= exp(Lp)*vo+(tau/2)*(N+exp(Lp)*No); %IFAM2
    ...
    N=alpha_m( us );
    No=alpha_m( uo );
    %mn=exp(L*tau)*mo+1/L*(exp(L*tau)-1)*N;  %% quella problematica ? la m
    %mn =mo + (tau/2) *(m_fun(mo,uo)+m_fun(ms,us)); % AM2
    mn= exp(Lp)*mo+(tau/2)*(N+exp(Lp)*No); %IFAM2
    ...
    N=alpha_f(us);
    No=alpha_f(uo);
    %fn=exp(L*tau)*fo+1/L*(exp(L*tau)-1)*N;
   % fn =fo + (tau/2) *(f_fun(fo,uo)+f_fun(fs,us));
     fn= exp(Lp)*fo+(tau/2)*(N+exp(Lp)*No); %IFAM2
    ...
    N=alpha_to( us );
    No=alpha_to( uo );
    %ton=exp(L*tau)*too+1/L*(exp(L*tau)-1)*N;
   % ton =too + (tau/2) *(to_fun(too,uo)+to_fun(tos,us));
    ton= exp(Lp)*too+(tau/2)*(N+exp(Lp)*No); %IFAM2
    ...
    N=X_inf( us )/tau_X( us) ;
    No=X_inf( uo )/tau_X( uo) ;
    %Xn=exp(L*tau)*Xo+1/L*(exp(L*tau)-1)*N;
    %Xn =Xo + (tau/2) *(X_fun(Xo,uo)+X_fun(Xs,us));
    Xn= exp(Lp)*Xo+(tau/2)*(N+exp(Lp)*No); %IFAM2 
    
    un=uo-tau*((1/2)*I_tot_o +(1/2)*I_tot_s);
...
for i=3:length(t)
   ...
   % us=uo-tau*(I_tot_o);
    un=uo-tau*((3/2)*I_tot_o +(-1/2)*I_tot_older); %AB2 lo terrei fisso attenzione se calcolassi un conun'altra strategia non avrei la dipendenza da olderk
    ...
    alpha_l= L-2*Lo+Lolder;
    beta_l= ((-t(i-1)-t(i-2))*L+ 2*(t(i)+t(i-2))*Lo+(-t(i)-t(i-1))*Lolder);
    gamma_l= t(i-1)*t(i-2)*L-2*t(i)*t(i-2)*Lo+t(i)*t(i-1)*Lolder;
    
    integrale_L=@(w)vpa((1/(2*tau^2))*((alpha_l/3)*w^3+(beta_l/2)*w^2+gamma_l*w),50);
    Lp= integrale_L(t(i))-integrale_L(t(i-1));  
        

    quadratic_L=@(w)vpa((1/(2*tau^2))*((alpha_l)*w^2+(beta_l)*w+gamma_l),50);
    
    alpha_n= N-2*No+Nolder;
    beta_n= ((-t(i-1)-t(i-2))*N+ 2*(t(i)+t(i-2))*No+(-t(i)-t(i-1))*Nolder);
    gamma_n= t(i-1)*t(i-2)*N-2*t(i)*t(i-2)*No+t(i)*t(i-1)*Nolder;
       
   quadratic_N=@(w)vpa((1/(2*tau^2))*((alpha_n)*w^2+beta_n*w+gamma_n),50);
    
     
    fu=@(w) exp(integrale_L(t(i))-integrale_L(w));
    fu_der=@(w)-quadratic_L(w)*fu(w);
    
    g=@(w) quadratic_N(w);
    g_der=@(w)(1/(2*tau^2))*(2*(alpha_n)*w+(beta_n));
    
    
     funzione=@(w)fu(w)*g(w);
     der_funzione=@(w)(fu(w)*g_der(w)+fu_der(w)*g(w));
  %   max(f_der(T(i-1),i-1)-(f(T(i),i)-f(T(i-1),i-1))/tau)
 ...
 alpha_l= L-2*Lo+Lolder;
  ...
  %   max(f_der(T(i-1),i-1)-(f(T(i),i)-f(T(i-1),i-1))/tau)
    ...
    un=uo-tau*( (5/12)*I_tot_n +(8/12)*I_tot_o +(-1/12)*I_tot_older); %AM3 lo terrei fisso
