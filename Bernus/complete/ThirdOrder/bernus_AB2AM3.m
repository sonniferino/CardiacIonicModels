function [u,t]=bernus_AB2AM3(num_steps)
...
for i=3:length(t)
    ...
    L=-1/tau_v((3/2)*uo-(1/2)*uolder);
    N=v_inf((3/2)*uo-(1/2)*uolder)/tau_v((3/2)*uo-(1/2)*uolder);
    vs=exp(L*dt)*vo+1/L*(exp(L*dt)-1)*N;
    %vn =vo + dt *v_fun(vo,uo);
    
    L=-alpha_m((3/2)*uo-(1/2)*uolder)-beta_m((3/2)*uo-(1/2)*uolder);
    N=alpha_m((3/2)*uo-(1/2)*uolder);
    ms=exp(L*dt)*mo+1/L*(exp(L*dt)-1)*N;  %% quella problematica ? la m
    %mn =mo + dt *m_fun(mo,uo);
    
    L=-alpha_f((3/2)*uo-(1/2)*uolder)-beta_f((3/2)*uo-(1/2)*uolder);
    N=alpha_f((3/2)*uo-(1/2)*uolder);
    fs=exp(L*dt)*fo+1/L*(exp(L*dt)-1)*N;
    %fn =fo + dt *f_fun(fo,uo);
    
    L=-alpha_to((3/2)*uo-(1/2)*uolder)-beta_to((3/2)*uo-(1/2)*uolder);
    N=alpha_to((3/2)*uo-(1/2)*uolder);
    tos=exp(L*dt)*too+1/L*(exp(L*dt)-1)*N;
    %ton=too+ dt *to_fun(too,uo);
    ...
    L=-alpha_m((5/12)*us +(8/12)*uo +(-1/12)*uolder )-beta_m( (5/12)*us +(8/12)*uo +(-1/12)*uolder );
    N=alpha_m( (5/12)*us +(8/12)*uo +(-1/12)*uolder );
    mn=exp(L*dt)*mo+1/L*(exp(L*dt)-1)*N;  %% quella problematica ? la m
    ...
    %%%% questo e' sbagliato. I_Na_s= I_Na_fun( us, ms) , vs );
    I_Na_s=I_Na_fun( us , ms , vs );
    I_Ca_s=I_Ca_fun( us,fs);
    I_to_s=I_to_fun( us,tos );
    I_K_s=I_K_fun( us, Xs );
