function [u,t,I_Na,I_Ca,I_to, I_K,I_K1,I_Nab,I_Cab,I_NaK,I_NaCa]=bernus_IFAB2IFAM3_also_gating(num_steps)
...
for i=2:2
    ...
    Lp= 0.5*dt*(Lo + L );  
    vs= exp(Lp)*vo+dt*exp(Lp)*No; %e' IFAB1
    ...
    Lp= 0.5*dt*(Lo + L );  
    ms= exp(Lp)*mo+dt*exp(Lp)*No; %e' IFAB1
   % ms=exp(L*dt)*mo+1/L*(exp(L*dt)-1)*N;  %% quella problematica ? la m
   % ms =mo + dt *m_fun(mo,uo); %e' AB1
    ...
    No=alpha_f(uo);
    Lp= 0.5*dt*(Lo + L );  
    fs= exp(Lp)*fo+dt*exp(Lp)*No; %e' IFAB1
    %fs=exp(L*dt)*fo+1/L*(exp(L*dt)-1)*N;
   % fs =fo + dt *f_fun(fo,uo); %e' AB1
    ...
    Lp= 0.5*dt*(Lo + L );  
    tos= exp(Lp)*too+dt*exp(Lp)*No; %e' IFAB1
    % tos=exp(L*dt)*too+1/L*(exp(L*dt)-1)*N;
    %tos=too+ dt *to_fun(too,uo); %e' AB1
    ...
    Lp= 0.5*dt*(Lo + L );  
    Xs= exp(Lp)*Xo+dt*exp(Lp)*No; %e' IFAB1
   % Xs=exp(L*dt)*Xo+1/L*(exp(L*dt)-1)*N;  %RL sulle gating
   % Xs= Xo + dt *X_fun(Xo,uo); %e' AB1
    ...
    vn= exp(Lp)*vo+(dt/2)*(N+exp(Lp)*No); %IFAM2
    ...
    mn= exp(Lp)*mo+(dt/2)*(N+exp(Lp)*No); %IFAM2
   ...
   % fn =fo + (dt/2) *(f_fun(fo,uo)+f_fun(fs,us));
     fn= exp(Lp)*fo+(dt/2)*(N+exp(Lp)*No); %IFAM2
   ...
   % ton =too + (dt/2) *(to_fun(too,uo)+to_fun(tos,us));
    ton= exp(Lp)*too+(dt/2)*(N+exp(Lp)*No); %IFAM2
    ...
    Xn= exp(Lp)*Xo+(dt/2)*(N+exp(Lp)*No); %IFAM2 
...
for i=3:length(t)
    ...
    us=uo-dt*((3/2)*I_tot_o +(-1/2)*I_tot_older); %AB2 lo terrei fisso
    ...
    vs= exp(Lp)*vo+(3/2)*dt*exp(Lp)*No+(-1/2)*dt*exp(Lg)*Nolder;          % voglio IFAB2
    ...
    ms= exp(Lp)*mo+(3/2)*dt*exp(Lp)*No+(-1/2)*dt*exp(Lg)*Nolder;          % voglio IFAB2
...
% %     
    ...
    fs= exp(Lp)*fo+(3/2)*dt*exp(Lp)*No+(-1/2)*dt*exp(Lg)*Nolder;          % voglio IFAB2
    ...
    tos= exp(Lp)*too+(3/2)*dt*exp(Lp)*No+(-1/2)*dt*exp(Lg)*Nolder;          % voglio IFAB2
    ...
    Xs= exp(Lp)*Xo+(3/2)*dt*exp(Lp)*No+(-1/2)*dt*exp(Lg)*Nolder;          % voglio IFAB2
    ...
    integrale=@(w)(1/(2*dt^2))*((alpha/3)*w^3+(beta/2)*w^2+gamma*w);
    
     Lp= integrale(t(i))-integrale(t(i-1));  
     Lg= integrale(t(i))-integrale(t(i-2));
    
    
    vn=exp(Lp)*vo+dt/12*(5*N + 8*exp(Lp)*No -1*exp(Lg)*Nolder) ;      %voglio IFAM3
   ...
   % mn =mo + dt *((5/12)*m_fun(ms,us)+(8/12)*m_fun(mo,uo)-(1/12)*m_fun(molder,uolder));
    ...
    mn=exp(Lp)*mo+dt/12*(5*N + 8*exp(Lp)*No -1*exp(Lg)*Nolder) ;      %voglio IFAM3
    ...
    fn=exp(Lp)*fo+dt/12*(5*N + 8*exp(Lp)*No -1*exp(Lg)*Nolder) ;      %voglio IFAM3
    ...
    ton=exp(Lp)*too+dt/12*(5*N + 8*exp(Lp)*No -1*exp(Lg)*Nolder) ;      %voglio IFAM3
   ...
   %Xn=exp(L*dt)*Xo+1/L*(exp(L*dt)-1)*N;
    ...
    Xn=exp(Lp)*Xo+dt/12*(5*N + 8*exp(Lp)*No -1*exp(Lg)*Nolder) ;      %voglio IFAM3
