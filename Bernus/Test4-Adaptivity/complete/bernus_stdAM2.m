function [u,t,v,m,f,to,X]=bernus_stdAM2(num_steps)  
...
for i=3:length(t)
   ...
   % us=uo-dt*(I_tot_o);
    un=uo-tau*((3/2)*I_tot_o +(-1/2)*I_tot_older); %AB2 lo terrei fisso ma se non dipendesse da older potrei farlo dal passo 2
    ...
    vn= (vo+tau/2*(Lo*vo+No+N))/(1-(tau/2)*L); %std AM2
    ...
    mn= (mo+tau/2*(Lo*mo+No+N))/(1-(tau/2)*L); %std AM2
    ...
    fn= (fo+tau/2*(Lo*fo+No+N))/(1-(tau/2)*L); %std AM2
    ...
    ton= (too+tau/2*(Lo*too+No+N))/(1-(tau/2)*L); %std AM2
    ...
    Xn= (Xo+tau/2*(Lo*Xo+No+N))/(1-(tau/2)*L); %std AM2
    
    I_Na_n=I_Na_fun( un , mn , vn );   %questi sono calcolati due volte
    I_Ca_n=I_Ca_fun( un,fn);
    I_to_n=I_to_fun( un,ton );
    I_K_n=I_K_fun( un, Xn );
    I_K1_n=I_K1_fun( un );
