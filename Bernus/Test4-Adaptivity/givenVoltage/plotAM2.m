function [u,t,v,m,f,to,X]=plotAM2(num_steps,tref,Vref)  
...
%Vsupp=Vref(1:100:end);
%plot(Vsupp,abs(alpha_m( Vsupp )./(-alpha_m(Vsupp)-beta_m( Vsupp ))))
...
toc
    ...
    tic
    L2=-alpha_m(VV)-beta_m( VV );
    toc
    
    %plot(tref,-alpha_m(Vref)-beta_m( Vref ))
    ...
    plot(tt,1./max(abs(L)))
    hold on
    %plot(tt,1./(abs(L1)),tt,1./(abs(L2)))
    disp('fine')
    toc
    %legend('vm','comanda v','comanda m')
    
    figure(2)
    plot(tt,abs(L1)-abs(L2))
