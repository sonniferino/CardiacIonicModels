function [u,t,mMID,mS]=bernus_singlevariablem(num_steps,tref,Vref)  
...
%     mn= (mo+tau/2*(Lo*mo+No+N))/(1-(tau/2)*L);  %AM2 
    ...
    derLinV=@(uu)(-alpha_m_der(uu)-beta_m_der( uu )):%/(u(i+1)-uo);
    derLinVo= derLinV(uo);
    derVint=(u(i+1)-uo)/tau;
    %i
    %(derLinV(um)*derVint)
    ...
    %return
      ...
      [tempo,mn] = ode23s(@(t,j)(L*j+N), tspan, mo);
    
   % [tempo,mn] = ode23s(@(t,j)(L*j+N+derLinVo*derVint*(t-tspan(1))), tspan, mo)
