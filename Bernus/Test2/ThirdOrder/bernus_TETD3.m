function [u,t,v,m,f,to,X]=bernus_TETD3(num_steps,tref,Vref)  
...
for i=3:length(t)
    ...
    integrale_L=@(w)(1/(2*tau^2))*((alpha_l/3)*w^3+(beta_l/2)*w^2+gamma_l*w);
    Lp= integrale_L(t(i))-integrale_L(t(i-1));  
        

    quadratic_L=@(w)(1/(2*tau^2))*((alpha_l)*w^2+(beta_l)*w+gamma_l);
   ...
   quadratic_N=@(w)(1/(2*tau^2))*((alpha_n)*w^2+beta_n*w+gamma_n);
