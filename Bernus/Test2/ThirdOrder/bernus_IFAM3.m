function [u,t,v,m,f,to,X]=bernus_IFAM3(num_steps,tref,Vref)  
...
for i=2:2%length(t)
    ...
    %vn =vo + (tau/2) *(v_fun(vo,uo)+v_fun(vs,un));  % AM2
    ...
    %mn =mo + (tau/2) *(m_fun(mo,uo)+m_fun(ms,un)); % AM2
   ...
   % fn =fo + (tau/2) *(f_fun(fo,uo)+f_fun(fs,un));
   ...
   % ton =too + (tau/2) *(to_fun(too,uo)+to_fun(tos,un));
    ...
    %Xn =Xo + (tau/2) *(X_fun(Xo,uo)+X_fun(Xs,un));
...
for i=3:length(t)
    ...
    integrale=@(w)(1/(2*tau^2))*((alpha/3)*w^3+(beta/2)*w^2+gamma*w);
    
     Lp= integrale(t(i))-integrale(t(i-1));  
     Lg= integrale(t(i))-integrale(t(i-2));
    
    
    vn=exp(Lp)*vo+tau/12*(5*N + 8*exp(Lp)*No -1*exp(Lg)*Nolder) ;      %voglio IFAM3
    
    
    %vn=exp(L*tau)*vo+1/L*(exp(L*tau)-1)*N;
    %vn =vo + tau *((5/12)*v_fun(vs,un)+(8/12)*v_fun(vo,uo)-(1/12)*v_fun(volder,uolder));
   ...
   %mn=exp(L*tau)*mo+1/L*(exp(L*tau)-1)*N;  %% quella problematica ? la m 
   % mn=ms+tau*( phi2 (L*tau) *(  Nstar - 2*No + Nolder ));%% quella problematica ? la m
   % mn =mo + tau *((5/12)*m_fun(ms,un)+(8/12)*m_fun(mo,uo)-(1/12)*m_fun(molder,uolder));
    
    
    mn=exp(Lp)*mo+tau/12*(5*N + 8*exp(Lp)*No -1*exp(Lg)*Nolder) ;      %voglio IFAM3
   ...
   Lp= integrale(t(i))-integrale(t(i-1));  
    ...
    %fn=exp(L*tau)*fo+1/L*(exp(L*tau)-1)*N;
    %fn =fo + tau *((5/12)*f_fun(fs,un)+(8/12)*f_fun(fo,uo)-(1/12)*f_fun(folder,uolder));
    fn=exp(Lp)*fo+tau/12*(5*N + 8*exp(Lp)*No -1*exp(Lg)*Nolder) ;      %voglio IFAM3
    ...
    %ton=exp(L*tau)*too+1/L*(exp(L*tau)-1)*N;
    % ton =too + tau *((5/12)*to_fun(tos,un)+(8/12)*to_fun(too,uo)-(1/12)*to_fun(toolder,uolder));
    ton=exp(Lp)*too+tau/12*(5*N + 8*exp(Lp)*No -1*exp(Lg)*Nolder) ;      %voglio IFAM3
   ...
   %Xn=exp(L*tau)*Xo+1/L*(exp(L*tau)-1)*N;
    %Xn =Xo + tau *((5/12)*X_fun(Xs,un)+(8/12)*X_fun(Xo,uo)-(1/12)*X_fun(Xolder,uolder)); %AM3
    Xn=exp(Lp)*Xo+tau/12*(5*N + 8*exp(Lp)*No -1*exp(Lg)*Nolder) ;      %voglio IFAM3
