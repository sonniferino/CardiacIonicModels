function [u,t,v,m,f,to,X]=bernus_AM2(num_steps,tref,Vref)  
...
for i=2:2%length(t)
    ...
    mn= (mo+tau*N)/(1-tau*L); %AM1
   
 
    L=-alpha_f( un )-beta_f(un );
    N=alpha_f(un);

   fn= (fo+tau*N)/(1-tau*L); %AM1
    ...
    ton= (too+tau*N)/(1-tau*L); %AM1  
    
    L=-1/tau_X( un );
    N=X_inf( un )/tau_X( un) ;

    
    Xn= (Xo+tau*N)/(1-tau*L); %AM1
