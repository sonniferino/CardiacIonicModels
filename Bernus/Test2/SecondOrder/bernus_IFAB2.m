function [u,t,v,m,f,to,X,Lv,Lm,Lf,Lto,LX,Nv,Nm,Nf,Nto,NX]=bernus_IFAB2(num_steps,tref,Vref)  
...
%V=[-100:0.001:100];
Lv=-1./tau_v(Vsupp);
Nv=v_inf(Vsupp)./tau_v(Vsupp);

Lm=-alpha_m(Vsupp)-beta_m( Vsupp );
Nm=alpha_m( Vsupp );

Lf=-alpha_f( Vsupp )-beta_f(Vsupp );
Nf=alpha_f(Vsupp);

Lto=-alpha_to( Vsupp )-beta_to( Vsupp );
Nto=alpha_to( Vsupp ); 

LX=-1./tau_X( Vsupp );
NX=X_inf( Vsupp )./tau_X( Vsupp) ;

subplot(1,5,1)
plot(Vsupp,abs(Nv./Lv))

subplot(1,5,2)
plot(Vsupp,abs(Nm./Lm))
subplot(1,5,3)
plot(Vsupp,abs(Nf./Lf))
subplot(1,5,4)
plot(Vsupp,abs(Nto./Lto))
subplot(1,5,5)
plot(Vsupp,abs(NX./LX))
...
for i=2:2%length(t)
    ...
    vn= exp(Lp)*vo+tau*exp(Lp)*No;%IFAB1 
    ...
    mn= exp(Lp)*mo+tau*exp(Lp)*No;%IFAB1 
   ...
   fn= exp(Lp)*fo+tau*exp(Lp)*No;%IFAB1 
    ...
    ton= exp(Lp)*too+tau*exp(Lp)*No;%IFAB1 
    ...
    Lp= 0.5*tau*( Lo + L );  
    Xn= exp(Lp)*Xo+tau*exp(Lp)*No;%IFAB1 
